...
export function GamePage() {
  ...
  useEffect(() => {
    ...
    if (!usuarioId || !marcarComoEnviado()) return;
    ...
    registrarPartida.mutate(
      ...
      {
        onSuccess: () => toast.success('Partida registrada!'),
        onError: () => toast.error('Nao foi possivel registrar a partida.'),
      },
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [jogo.status]);

  function handleTentarLetra(letra: string) {
    const acertou = jogo.palavra.includes(letra.toUpperCase());
  ...
  return (
    <main className="min-h-screen">
      <Header />
      <div className="mx-auto flex max-w-2xl flex-col items-center gap-6 px-4 pb-8">
        ...
        {jogoFinalizado ? (
          ...
          <Teclado
            ...
            onTentarLetra={handleTentarLetra}
          />
        )}
